...
/** Product branding — DirotiQ CRM */
export const APP_NAME = 'DirotiQ CRM';
export const APP_WORDMARK = 'DirotiQ';
export const APP_DESCRIPTOR = 'CRM';
export const APP_SHORT_NAME = 'DirotiQ';
export const APP_LOGO_SRC = '/logos/lujunal_ops_mark_512.png';
...
export const TEAM_CALL_BRAND_NAME = readViteEnv('VITE_JITSI_APP_NAME', 'DirotiQ CRM Calls');
...
export const BRAND_CLASSIFICATION_CONFIG: Record<string, ClassificationConfig> = {
  optimaviz: {
    enabled: true,
    auto_promote_triggers: [
      { id: 'reply', name: 'Email Reply', trigger_type: 'communication_reply', enabled: true, auto_promote: true },
      { id: 'signup', name: 'Signup Complete', trigger_type: 'signup_complete', enabled: true, auto_promote: true },
      { id: 'trial', name: 'Trial Started', trigger_type: 'purchase_complete', enabled: true, auto_promote: true },
    ],
    manual_promote_allowed: true,
    manual_demote_allowed: true,
    verified_starting_stage: 'Subscribed',
  },
  taskgo: {
    enabled: true,
    auto_promote_triggers: [
      { id: 'verified', name: 'ABN Verified', trigger_type: 'info_provided', enabled: true, auto_promote: true },
      { id: 'signup', name: 'Platform Signup', trigger_type: 'signup_complete', enabled: true, auto_promote: true },
    ],
    manual_promote_allowed: true,
    manual_demote_allowed: false,
    verified_starting_stage: 'Approved Contractor',
  },
  idao: {
    enabled: true,
    auto_promote_triggers: [
      { id: 'reply', name: 'Email Reply', trigger_type: 'communication_reply', enabled: true, auto_promote: true },
      { id: 'registered', name: 'Registered for Training', trigger_type: 'signup_complete', enabled: true, auto_promote: true },
      { id: 'attended', name: 'Attended Session', trigger_type: 'meeting_attended', enabled: true, auto_promote: true },
    ],
    manual_promote_allowed: true,
    manual_demote_allowed: true,
    verified_starting_stage: 'Registered',
  },
  optimaclean: {
    enabled: true,
    auto_promote_triggers: [
      { id: 'reply', name: 'Email Reply', trigger_type: 'communication_reply', enabled: true, auto_promote: true },
      { id: 'purchase', name: 'Service Booked', trigger_type: 'purchase_complete', enabled: true, auto_promote: true },
    ],
    manual_promote_allowed: true,
    manual_demote_allowed: true,
    verified_starting_stage: 'Active Service',
  },
  nestwise: {
    enabled: true,
    auto_promote_triggers: [
      { id: 'reply', name: 'Email Reply', trigger_type: 'communication_reply', enabled: true, auto_promote: true },
      { id: 'meeting', name: 'Property Viewing', trigger_type: 'meeting_attended', enabled: true, auto_promote: true },
      { id: 'info', name: 'Form Submitted', trigger_type: 'info_provided', enabled: true, auto_promote: true },
    ],
    manual_promote_allowed: true,
    manual_demote_allowed: true,
    verified_starting_stage: 'Owner Approved',
  },
};
...
export const findLeadDateHeader = (headers: string[]) => {
  ...
  return headers.find(header => {
    ...
    return (
      ...
      compact.includes('entrydate') ||
      compact.includes('importdate') ||
      compact.includes('dateadded') ||
      compact.includes('created') ||
      compact.includes('date') ||
      compact.includes('datetime') ||
      compact.includes('date_time') ||
      compact.includes('timecreated') ||
      compact.includes('createdtime')
    );
  }) || '';
};
  ...
  export const parseLeadDateValue = (value: any): Date | null => {
    ...
    if (monthNameBeforeDate) {
      ...
      if (mIndex >= 0) {
        const date = new Date(Number(y), mIndex, Number(d));
        if (!Number.isNaN(date.getTime())) return date;
      }
    }
    ...
    if (timeWithDate) {
      ...
      const datePart = parseLeadDateValue(dateStr);
      if (datePart) {
...
export const DEFAULT_SNAPSHOT_CARDS: Record<string, SnapshotCardConfig[]> = {
  taskgo: [
    { id: 'tg-abn', brand_id: 'taskgo', label: 'Verified ABN Contractors', fieldKey: 'abn_number', target: 40, unit: 'ABNs', icon: 'fa-file-invoice', color: '#10B981', active: true },
    { id: 'tg-platform', brand_id: 'taskgo', label: 'Platform Registered Contractors', fieldKey: 'segment', matchValue: 'all_platform', target: 100, unit: 'Profiles', icon: 'fa-users', color: '#3B82F6', active: true },
    { id: 'tg-total', brand_id: 'taskgo', label: 'Active Roster Pipeline Goals', fieldKey: '__total__', target: 150, unit: 'Total', icon: 'fa-user-clock', color: '#8B5CF6', active: true }
  ],
  idao: [
    { id: 'id-training', brand_id: 'idao', label: '3 Day Training', fieldKey: 'segment', matchValue: 'training_leads', target: 30, unit: 'Leads', icon: 'fa-graduation-cap', color: '#8B5CF6', active: true },
    { id: 'id-referrals', brand_id: 'idao', label: 'Optimaviz Referrals', fieldKey: 'segment', matchValue: 'optimaviz_referrals', target: 15, unit: 'Leads', icon: 'fa-project-diagram', color: '#3B82F6', active: true },
    { id: 'id-other', brand_id: 'idao', label: 'Other Services', fieldKey: 'segment', matchValue: 'other_services', target: 10, unit: 'Leads', icon: 'fa-briefcase', color: '#10B981', active: true }
  ],
  optimaclean: [
    { id: 'oc-clients', brand_id: 'optimaclean', label: 'Corporate Client Accounts', fieldKey: 'segment', matchValue: 'clients', target: 25, unit: 'Clients', icon: 'fa-user-tie', color: '#10B981', active: true },
    { id: 'oc-workers', brand_id: 'optimaclean', label: 'Partner Cleaner Roster Count', fieldKey: 'segment', matchValue: 'workers', target: 50, unit: 'Cleaners', icon: 'fa-broom', color: '#F59E0B', active: true }
  ],
  nestwise: [
    { id: 'nw-hosting', brand_id: 'nestwise', label: 'Airbnb Hosting Pipeline', fieldKey: 'segment', matchValue: 'airbnb_hosting_support', target: 25, unit: 'Owners', icon: 'fa-bed', color: '#FD5C63', active: true },
    { id: 'nw-marketing', brand_id: 'nestwise', label: 'Marketing Package Leads', fieldKey: 'segment', matchValue: 'property_marketing', target: 25, unit: 'Listings', icon: 'fa-bullhorn', color: '#2563EB', active: true },
    { id: 'nw-care', brand_id: 'nestwise', label: 'Care / Inspection Subscribers', fieldKey: 'segment', matchValue: 'property_care', target: 30, unit: 'Properties', icon: 'fa-clipboard-check', color: '#8B5CF6', active: true },
    { id: 'nw-security', brand_id: 'nestwise', label: 'Security Monitoring Leads', fieldKey: 'segment', matchValue: 'property_security', target: 18, unit: 'Properties', icon: 'fa-shield-halved', color: '#10B981', active: true }
  ]
};
...
export function DirotiQLogo({ size = 48 }: { size?: number }) {
  return (
    <img
      src={APP_LOGO_SRC}
      alt={APP_NAME}
      ...
      style={{
        ...
        objectFit: 'contain',
        display: 'block',
        filter: 'drop-shadow(0px 4px 10px rgba(37, 99, 235, 0.12))',
      }}
      draggable={false}
    />
  );
...
/** @deprecated Use DirotiQLogo — kept so older imports keep working. */
export function LujuNalLogo({ size = 48 }: { size?: number }) {
  return <DirotiQLogo size={size} />;
}
