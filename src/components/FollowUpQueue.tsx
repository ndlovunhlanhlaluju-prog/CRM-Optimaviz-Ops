...
export default function FollowUpQueue({ leads, brands, onOpenLead, onQuickCall }: FollowUpQueueProps) {
  ...
  const rows = useMemo(() => {
    return leads
      ...
      .filter(item => !dismissedIds.has(item.lead.id))
  ...
  const brandCounts = useMemo(() => {
    return filteredRows.reduce<Record<string, number>>((acc, { lead }) => {
      const brand = brands.find(item => item.id === lead.brand_id);
      const key = brand?.name || lead.brand_name || lead.brand_id || 'Unknown brand';
      acc[key] = (acc[key] || 0) + 1;
      return acc;
    }, {});
  }, [filteredRows, brands]);
  ...
  return (
    <section className="follow-up-queue">
      <div className="follow-up-queue__header">
        ...
        <div className="follow-up-queue__tabs">
          ...
          {filteredRows.length > 0 && (
            <button type="button" className="follow-up-queue__clear" onClick={() => setDismissedIds(prev => new Set([...prev, ...filteredRows.map(item => item.lead.id)]))}>
              Clear view
            </button>
          )}
        </div>
      ...
      {filteredRows.length > 0 && (
        <div className="follow-up-queue__brand-strip">
          {Object.entries(brandCounts).slice(0, 6).map(([brandName, count]) => (
            <span key={brandName}>{brandName}<b>{count}</b></span>
          ))}
        </div>
      )}
          ...
          filteredRows.slice(0, 12).map(({ lead, due }) => {
            ...
            return (
              <div key={lead.id} className="follow-up-queue__row">
                <div className="follow-up-queue__avatar" style={{ background: `${brand?.color || '#0f766e'}22`, color: brand?.color || '#0f766e' }}>
                ...
                <div className="follow-up-queue__actions">
                  ...
                  <button type="button" onClick={() => setDismissedIds(prev => new Set(prev).add(lead.id))} title="Clear from this queue">
